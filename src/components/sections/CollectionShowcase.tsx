...
function BadgeLabel({ stock, isPreOrder }: { stock: number; isPreOrder?: boolean }) {
  if (stock === 0 && isPreOrder) {
    return (
      <span className="absolute left-4 bottom-4 z-10 border border-accent/40 bg-background/60 px-3 py-1.5 text-[10px] font-light tracking-[2.5px] uppercase text-accent backdrop-blur-sm">
  ...
  if (stock === 0) {
    return (
      <span className="absolute left-4 bottom-4 z-10 border border-white/15 bg-background/60 px-3 py-1.5 text-[10px] font-light tracking-[2.5px] uppercase text-white/50 backdrop-blur-sm">
        Sold Out
      </span>
    );
  }
  if (stock <= 3) {
    return (
      <span className="absolute left-4 bottom-4 z-10 border border-red-500/40 bg-background/60 px-3 py-1.5 text-[10px] font-light tracking-[2.5px] uppercase text-red-400/90 backdrop-blur-sm">
        Almost Gone
      </span>
    );
  }
